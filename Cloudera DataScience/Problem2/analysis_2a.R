...
aggDf <- DT[, j = list(sumServices = sum(Services),sumTotalCost = sum(TotalCost),varCost = var(Cost), 
                       ...
                       sumProc = sum(APC)+sum(DRG)), by = 'Provider.Id']
aggDf <- na.omit(aggDf)
Provider.Id.names = as.vector(aggDf$Provider.Id)
...
mydata.names <- subset(aggDf,  select = c('Provider.Id'))
...
for (i in 2:200) wss[i] <- sum(kmeans(data, centers=i)$withinss)
plot(1:200, wss, type="b", xlab="Number of Clusters", ylab="Within groups sum of squares")
...
row.names(mydata) <- Provider.Id.names
